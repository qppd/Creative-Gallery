                    ...
                    <div class="row">
                        ...
                        @foreach ($arts as $index => $art)
                            <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                                ...
                                <div class="card @if (($art['has_active_bid'] == 0 && $art['art_status'] == 2) || ($art['art_status'] == 1 && session('role') == 1)) art-card @endif"
...
document.addEventListener('contextmenu', event => event.preventDefault());
        ...
        function sendSearchRequest(query) {
            ...
            var url = "/search?_token=" + encodeURIComponent(csrfToken) + "&query=" + encodeURIComponent(query);

            xhr.open("GET", url, true);
            xhr.send();
        }

        function updateArtList(arts) {
            var container = document.querySelector('.content .container-fluid .row');
            container.innerHTML = '';

            arts.forEach(function(art) {
                var artElement = createArtCard(art);
        ...
        function createArtCard(art) {
            ...
            div.classList.add('col-lg-3', 'col-md-3', 'col-sm-12', 'col-xs-12');

            var cardClass = (art.art_status == 2 || (art.art_status == 1 && '{{ session('role') }}' == 1)) ? 'art-card' :
                '';

            console.log(art.starting_price)
        ...
        document.addEventListener('click', function(e) {
            if (e.target.closest('.art-card')) {
                ...
                if (user.subscription_status == false && user.role == 2) {
                    ...
                    xhr.onreadystatechange = function() {
                        if (this.readyState == 4 && this.status == 200) {
                            ...
                            offers.forEach(element => {
                                ...
                                productInfo.classList.add('product-info', 'd-flex', 'flex-grow-1',
                                ...
                                if (x == 0) {
                                    ...
                                    btnGroupContainer.classList.add('d-flex', 'align-items-start',
                                        'ml-auto');
                                    ...
                                    rejectBtn.classList.add('btn', 'btn-danger', 'btn-xs', 'reject');
                                    ...
                                    acceptBtn.classList.add('btn', 'btn-success', 'btn-xs', 'accept');
                    ...
                    var url = "/home/offers";
                    var params = "id=" + card.data('id') + "&_token=" + encodeURIComponent('{{ csrf_token() }}');
                    ...
                    $('#view_as_buyer #enthusiast_duration').html(card.data('duration'));
